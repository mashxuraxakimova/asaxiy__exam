    ...
    <div className="single_route container">
      ...
      <div className="single-route__content">
        <div className="sr__contentTop">
          {heart?.some((i) => i.id === data.id) ? (
            ...
            <button onClick={() => dispatch(addToHeart(data))}>
              <span>I want to check</span>
            </button>
          )}
        </div>
        <h3 className="sr__contentTitle">{data?.title}</h3>
        <p className="sr__contentData">
          Sotuvchi: <span>{"Mike Den"}</span>
        </p>
        <p className="sr__contentData">
          Chegirma: <span>{"1 kun, bepul"}</span>
        </p>
        <hr />
        <div className="sr__contentMiqdor">
          ...
          <div className="cart__card-num">
            <button
              ...
              className="cart__num-btn"
            >
              +
            </button>
            <span>{data.soni}</span>
            {console.log(data.soni)}
            <button
              onClick={() =>
                data.soni <= 1
                  ? dispatch(removeFromCart(data))
                  : dispatch(decFromCart(data))
              }
              className="cart__num-btn"
            >
              -
        ...
        <div className="sr__btns">
          ...
          <button className="sr__btn">Tugmani 1 bosganda xarid qilish</button>
        </div>
      </div>
    </div>
  );
